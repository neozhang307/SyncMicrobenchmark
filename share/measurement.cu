...
//2. measure latencys in ns (single GPU grid sync)
...
{
	{
		...
		{
			clock_gettime(CLOCK_REALTIME, &tsstart);
			run_func(kernel_func,blockPerGPU,threadPerBlock,KernelArgs, gpu_count);
			cudaDeviceSynchronize();
...
//3. mearsure kernel latency (single GPU)
...
{
	{
		...
		for(int i=0; i<SIZE; i++)
		{
			...
			clock_gettime(CLOCK_REALTIME, &tsstart);
			//launch
			run_func(kernel_func,blockPerGPU,threadPerBlock);
			clock_gettime(CLOCK_REALTIME, &tsend);
			//execution
			cudaDeviceSynchronize();
 			clock_gettime(CLOCK_REALTIME, &tsendop);
