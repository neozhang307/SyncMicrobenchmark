...
#define TEST_ADDITIONAL_LATENCY(callfunc, kernel,basicDEP, moreDEP, gpu_count, block_perGPU,thread_perBlock); \
	measureKernelLatency<gpu_count>(result, callfunc##_##basicDEP, kernel,block_perGPU,thread_perBlock);\
	measureKernelLatency<gpu_count>(result+1, callfunc##_##moreDEP, kernel,block_perGPU,thread_perBlock);\
...
#define TEST_FUSED_KERNEL_1V16_DIFERENCE(callfunc, baisckernel,fusedkernel, basicDEP, moreDEP, gpu_count,block_perGPU,thread_perBlock,idea_basic_workload) \
	measureKernelLatency<gpu_count>(result,   callfunc##_1,  fusedkernel, block_perGPU, thread_perBlock);\
	measureKernelLatency<gpu_count>(result+1, callfunc##_16, baisckernel, block_perGPU, thread_perBlock);\
	measureKernelLatency<gpu_count>(result+2, callfunc##_1,  baisckernel, block_perGPU, thread_perBlock);\
